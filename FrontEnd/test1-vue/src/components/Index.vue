...
export default {
  ...
  methods: {
    ...
    getCourseList(){ 
      return this.axios
      .get("http://localhost:9000/course/course/getAllCourse")
      .then((result) => {
        //console.log(result);
        this.courseList = result.data;
      }).catch( (error)=>{
        this.$message.error("获取全部课程信息失败！");
      } );
    },
    // 获取登录用户购买成功的课程信息
    getMyCourseList(){
      return this.axios
      .get("http://localhost:9000/course/course/getMyCoursesById",{
        Headers:{
         'Content-Type':'text/plain'
        },
        params:{
          ids:qs.stringify(this.myCourseIds)
        }
      })
      .then((result) => {
        //console.log(result);
    ...
    // 1.去订单微服务 获取登录用户已经成功购买的课程id
    getMyCourseIds(){
      return this.axios
      .get("http://localhost:9000/order/order/getOKOrderCourseIds/"+this.user.content.id)
      .then((result) => {
        this.myCourseIds = result.data; // 购买的全部课程id
        console.log(this.myCourseIds);
        if(this.myCourseIds.length !=0){
          this.getMyCourseList(); // 根据课程id，查询课程详情
        }
        
      }).catch( (error)=>{
        this.$message.error("获取已购买的课程ID失败！");
      });
    }
  },
